...
class QueueNotEmptyAutoScaler extends BaseAutoScaler implements AutoScaler
{
    ...
    {
        $allowedOverflow = $this->config->getParameter(self::PARAM_ALLOWED_OVERFLOW) > 0 ? $this->config->getParameter(self::PARAM_ALLOWED_OVERFLOW) : ($this->config->getParameter(self::PARAM_ALLOWED_OVERFLOW_PER_PROC) * $autoScaleRequest->numProcs());
